import 'package:flutter/material.dart';

//For handyman
final bgColor = Color(0xffF6BD60);
final bgColorTwo = Color(0xff946713);//Mush
final bgColorThree = Color(0xffffaf49);
final textColor = Color(0xff84A59D);
final pinkColor = Color(0xffF5CAC3);
final bgCard = Color(0xffF7EDE2);
final cardTile = Colors.grey[150];
final blueGrey = Colors.blueGrey;
final green  = Color(0xff54938b);
final brown = Color(0xffd2a67a);
final blue = Color(0xff7a92c5);
final outline = Color(0xff445D5E);

//https://assets8.lottiefiles.com/packages/lf20_gatzs89r.json

//For Mush
// final bgColor = Color(0xffF6BD60);
// // final bgColor = Color(0xff946713);//Mush
// final textColor = Color(0xff84A59D);
// final pinkColor = Color(0xffF5CAC3);
// final bgCard = Color(0xffF7EDE2);

//#dbc791
// 0xffdbc791
//0xff
//#ffaf49
//0xffffaf49
